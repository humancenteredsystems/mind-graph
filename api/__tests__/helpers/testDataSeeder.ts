...
export class TestDataSeeder {
  ...
  async seedTestData(): Promise<boolean> {
    ...
    const testClient = DgraphTenantFactory.createTestTenant(); // Still need this for direct GraphQL
    ...
    try {
      ...
      await (global as any).testUtils.wait(500); // Wait 500ms
      ...
      const hierarchyCreationResp = await callApi(
          this.API_BASE_URL,
          "/hierarchy",
          this.ADMIN_API_KEY,
          "POST",
          hierarchyData,
          tenantHeaders // Send tenant header even for admin endpoint if it supports it
      );
      ...
      if (!hierarchyCreationResp.success || !hierarchyCreationResp.data?.id) {
        ...
        if (hierarchyCreationResp.details) console.error("Details:", hierarchyCreationResp.details);
      ...
      const createdLevelIdsMap: { [key: number]: string } = {}; // Map levelNumber to actual created ID
      ...
      for (const levelData of levelsToCreate) {
        ...
        const levelCreationResp = await callApi(
            this.API_BASE_URL,
            "/hierarchy/level",
            this.ADMIN_API_KEY,
            "POST",
        ...
        if (!levelCreationResp.success || !levelCreationResp.data?.id) {
          ...
          if (levelCreationResp.details) console.error("Details:", levelCreationResp.details);
