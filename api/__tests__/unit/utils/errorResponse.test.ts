import {
  ...
  MultiTenantNotSupportedError,
  createEnterpriseErrorResponse,
  createNamespaceErrorResponse,
  createMultiTenantErrorResponse,
  FallbackBehavior,
  getFallbackBehavior
...
describe('Enterprise Error Handling - Issue #18', () => {
  describe('Enterprise Error Classes', () => {
    it('should create EnterpriseFeatureNotAvailableError with correct message and context', () => {
      const error = new EnterpriseFeatureNotAvailableError('Multi-tenant operations', {
    ...
    it('should create NamespaceNotSupportedError with namespace-specific context', () => {
      const error = new NamespaceNotSupportedError('GraphQL execution', '0x1', 'Custom suggestion');
    ...
    it('should create MultiTenantNotSupportedError with multi-tenant context', () => {
      const error = new MultiTenantNotSupportedError('tenant creation');
  ...
  describe('Error Response Builders', () => {
    it('should create consistent Enterprise error response format', () => {
      const response = createEnterpriseErrorResponse(
        'Multi-tenant operations',
        'tenant creation',
        { namespacesSupported: false, enterpriseDetected: false }
      );
    ...
    it('should create namespace-specific error response', () => {
      const response = createNamespaceErrorResponse(
        'Schema fetch',
        '0x2',
        { namespacesSupported: false, enterpriseDetected: true }
      );
    ...
    it('should create multi-tenant error response', () => {
      const response = createMultiTenantErrorResponse('tenant deletion');
    ...
    it('should detect different deployment modes correctly', () => {
      // OSS mode
      const ossResponse = createEnterpriseErrorResponse('test', 'test', { 
      ...
      const enterpriseResponse = createEnterpriseErrorResponse('test', 'test', { 
        namespacesSupported: false, 
      ...
      const multiTenantResponse = createEnterpriseErrorResponse('test', 'test', { 
  ...
  describe('Fallback Behavior Consistency', () => {
    it('should define correct fallback behavior for operation types', () => {
      expect(getFallbackBehavior('CREATE')).toBe(FallbackBehavior.FAIL_HARD);
      expect(getFallbackBehavior('READ')).toBe(FallbackBehavior.DEGRADE_GRACEFULLY);
      expect(getFallbackBehavior('ADMIN')).toBe(FallbackBehavior.FAIL_WITH_CONTEXT);
    });

    it('should have consistent fallback behavior values', () => {
      expect(FallbackBehavior.FAIL_HARD).toBe('FAIL_HARD');
      expect(FallbackBehavior.DEGRADE_GRACEFULLY).toBe('DEGRADE_GRACEFULLY');
      expect(FallbackBehavior.FAIL_WITH_CONTEXT).toBe('FAIL_WITH_CONTEXT');
    });
  });

  describe('Error Type Constants', () => {
    it('should have correct Enterprise error type values', () => {
      expect(ErrorType.ENTERPRISE_FEATURE_NOT_AVAILABLE).toBe('ENTERPRISE_FEATURE_NOT_AVAILABLE');
      expect(ErrorType.NAMESPACE_NOT_SUPPORTED).toBe('NAMESPACE_NOT_SUPPORTED');
      expect(ErrorType.MULTI_TENANT_NOT_AVAILABLE).toBe('MULTI_TENANT_NOT_AVAILABLE');
    });
  });
});
