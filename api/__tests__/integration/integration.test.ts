...
}) as any as MockAdaptiveTenantFactory; // Assert the type of the result of jest.mock
...
describe('Integration /api/traverse', () => {
  ...
  it('should filter out dangling edges during traversal', async () => {
    ...
    // Define a type for the outgoing edge structure
    type OutgoingEdge = { to: { id: string } | null };
    const targets = node.outgoing.map((e: OutgoingEdge) => e.to?.id).filter((id: string | undefined) => id !== undefined); // Add type annotation for filter parameter
