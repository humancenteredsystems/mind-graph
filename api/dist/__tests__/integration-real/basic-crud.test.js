...
const conditionalDescribe = global.DGRAPH_ENTERPRISE_AVAILABLE ? describe : describe.skip;
    ...
    describe('Node Creation', () => {
        it('should create a node in test tenant namespace', async () => {
          ...
          `,
            ...
            const result = await (0, realTestHelpers_1.verifyInTestTenant)(`
      ...
      `, { id: nodeData.id });
        ...
        it('should create multiple nodes in batch', async () => {
            const nodeData1 = (0, realTestHelpers_1.createTestNodeData)({ label: 'Batch Node 1', type: 'concept' });
            const nodeData2 = (0, realTestHelpers_1.createTestNodeData)({ label: 'Batch Node 2', type: 'example' });
            const response = await (0, realTestHelpers_1.testRequest)(server_1.default)
                .post('/api/mutate')
                .set('X-Hierarchy-Id', 'test-hierarchy-1')
                .send({
          ...
          `,
            ...
            const result1 = await (0, realTestHelpers_1.verifyInTestTenant)(`query { getNode(id: "${nodeData1.id}") { id label } }`);
            const result2 = await (0, realTestHelpers_1.verifyInTestTenant)(`query { getNode(id: "${nodeData2.id}") { id label } }`);
