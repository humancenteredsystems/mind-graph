...
describe('SchemaRegistry Service', () => {
    ...
    describe('getAllSchemas', () => {
        it('should return all schemas from registry', async () => {
            ...
            expect(fs.promises.readFile).toHaveBeenCalledWith(expect.stringContaining('schema_registry.json'), 'utf8');
        });
    ...
    describe('addSchema', () => {
        it('should add new schema successfully', async () => {
            ...
            const result = await (0, schemaRegistry_1.addSchema)(schemaInfo, schemaContent);
        ...
        it('should throw error when schema ID already exists', async () => {
            ...
            fs.promises.readFile.mockResolvedValueOnce(JSON.stringify(mockRegistry));
            await expect((0, schemaRegistry_1.addSchema)(schemaInfo, 'content')).rejects.toThrow('Schema ID already exists');
        });
        it('should handle production schema marking', async () => {
            ...
            fs.promises.writeFile.mockResolvedValue();
            fs.promises.mkdir.mockResolvedValue();
            const result = await (0, schemaRegistry_1.addSchema)(schemaInfo, 'content');
            expect(result.is_production).toBe(true);
            // Should have written registry with old production unmarked
            const registryCall = fs.promises.writeFile.mock.calls.find((call) => call[0].includes('schema_registry.json'));
    ...
    describe('updateSchema', () => {
        it('should update schema metadata successfully', async () => {
            ...
            fs.promises.readFile.mockResolvedValueOnce(JSON.stringify(mockRegistry));
            fs.promises.writeFile.mockResolvedValue();
            const result = await (0, schemaRegistry_1.updateSchema)('schema1', updates);
            expect(result.name).toBe('New Name');
            expect(result.description).toBe('Updated description');
            expect(result.updated_at).toBeDefined();
            expect(fs.promises.writeFile).toHaveBeenCalledWith(expect.stringContaining('schema_registry.json'), expect.any(String), 'utf8');
        });
        it('should update schema content when provided', async () => {
            ...
            fs.promises.readFile.mockResolvedValueOnce(JSON.stringify(mockRegistry));
            fs.promises.writeFile.mockResolvedValue();
            await (0, schemaRegistry_1.updateSchema)('schema1', updates, newContent);
            expect(fs.promises.writeFile).toHaveBeenCalledTimes(2); // Registry + content
            expect(fs.promises.writeFile).toHaveBeenCalledWith(expect.stringContaining('schemas/test.graphql'), newContent, 'utf8');
        });
        it('should throw error when schema does not exist', async () => {
            const mockRegistry = { schemas: [] };
            fs.promises.readFile.mockResolvedValueOnce(JSON.stringify(mockRegistry));
            await expect((0, schemaRegistry_1.updateSchema)('nonexistent', {})).rejects.toThrow('Schema not found');
        });
    });
});
