...
export class TestRunner extends EventEmitter {
  ...
  private static readonly JEST_TEXT_PATTERNS = {
   ...
   */
  private parseJestJsonOutput(output: string): TestSummary | null {
    try {
   ...
   */
  private parseJestTextOutput(output: string): TestSummary | null {
    try {
   ...
   */
  private createTestSummary(passed: number, failed: number, total: number, suites: number): TestSummary {
    return {
  ...
  private parseIntegrationRealOutput(result: TestRunResult, output: string): void {
    ...
    try {
      ...
      if (output.includes('PASS') || output.includes('FAIL') || output.includes('✓') || output.includes('✗')) {
        ...
        result.summary = this.createTestSummary(
          output.includes('PASS') || output.includes('✓') ? 1 : 0,
          output.includes('FAIL') || output.includes('✗') ? 1 : 0,
          1,
          1
        );
