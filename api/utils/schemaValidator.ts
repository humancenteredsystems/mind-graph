...
export class SchemaValidator {
   ...
   */
  static async verifySchemaInNamespace(namespace: string): Promise<{
    success: boolean;
    details: string;
    missingTypes?: string[];
    introspectionWorking?: boolean;
    coreTypesPresent?: boolean;
    ...
    try {
      ...
      try {
        ...
        return {
          success: false,
          details: `Schema introspection failed: ${(introspectionError as Error).message}`,
          introspectionWorking: false
        };
      }
      ...
      for (const typeName of requiredCoreTypes) {
        try {
          ...
          missingCoreTypes.push(typeName);
          coreTypesPresent = false;
          console.log(`[SCHEMA_VALIDATOR] ❌ Error checking core type ${typeName}: ${(typeError as Error).message}`);
        }
      }
      ...
      const missingHierarchyTypes: string[] = [];
      ...
      for (const typeName of requiredHierarchyTypes) {
        try {
          ...
          missingHierarchyTypes.push(typeName);
          hierarchyTypesPresent = false;
          console.log(`[SCHEMA_VALIDATOR] ❌ Error checking hierarchy type ${typeName}: ${(typeError as Error).message}`);
        }
      }
      ...
      return {
        success: false,
        details: `Unexpected error during schema verification: ${(unexpectedError as Error).message}`,
        introspectionWorking: false
      };
    }
  }
   ...
   */
  static async waitForSchemaAvailability(
    namespace: string, 
    maxWaitMs: number = 10000, 
    pollIntervalMs: number = 500
  ): Promise<{ success: boolean; details: string; waitedMs: number }> {
   ...
   */
  static validateSchemaContent(schemaContent: string): {
    valid: boolean;
    errors: string[];
    warnings: string[];
  } {
