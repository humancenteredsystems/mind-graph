...
// Interface definitions for backup data structures
...
export class TenantMigration {
  ...
  async backupTenant(tenantId: string, backupPath: string | null = null): Promise<BackupResult> {
    ...
    try {
      ...
      const data: BackupData = await tenant.executeGraphQL(exportQuery);
  ...
  private async restoreHierarchy(tenant: any, hierarchy: Hierarchy): Promise<void> {
    try {
      const mutation = `
  ...
  private async restoreNodes(tenant: any, nodes: Node[]): Promise<void> {
    try {
      const mutation = `
  ...
  private async restoreEdges(tenant: any, edges: Edge[]): Promise<void> {
    try {
      const mutation = `
